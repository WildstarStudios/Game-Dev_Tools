...
class ADVANCED_GLB_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # OBJ Up Axis when relevant
        if scene_props.export_format == 'OBJ':
            quick_box.prop(scene_props, "obj_up_axis", expand=True)
        ...
        mod_grid.label(text="• -sk → Skip collection (strict)")
...
# ===== UTILITY FUNCTIONS =====
# [All the utility functions remain exactly the same as in your original code]
# get_extension, check_operator_exists, get_available_export_operators,
# export_obj_compat, get_collection_center, move_collection_to_origin,
# restore_collection_positions, parse_modifiers, validate_sk_modifiers,
# get_track_file_path, get_blend_track_file_path, load_track_data,
# save_track_data, update_track_file, find_orphaned_files, cleanup_orphaned_files,
# get_final_export_path, ensure_directory_exists, get_collection_for_object,
# resolve_export_directory, get_quick_summary, get_detailed_summary,
# find_collection_export_roots, get_object_exclusion_reason,
# get_collection_exclusion_reason, should_export_object, should_export_collection,
# move_to_3d_cursor, restore_original_position, export_glb
    ...
    try:
        ...
        if bpy.app.version >= (4, 0, 0):
            export_params = {
                ...
                'global_scale': 1.0,
                'forward_axis': 'Y',
                'up_axis': scene_props.obj_up_axis
            }
