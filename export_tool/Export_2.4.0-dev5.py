            ...
            for line in issues_lines:
                if line.strip():
                    if line.startswith('ERROR:'):
                        ...
                        row.label(text=line[6:].strip())
                    elif line.startswith('WARNING:'):
                        row = box.row()
                        row.label(text="", icon='INFO')
                        row.label(text=line[8:].strip())
                    else:
                        box.label(text=line)
        
        layout.separator()
        layout.label(text="Please fix these issues before exporting.", icon='INFO')
...
class ADVANCED_GLB_OT_validate_export_modifiers(bpy.types.Operator):
    ...
    def format_issues_for_display(self, issues):
        ...
        if errors:
            ...
            for error in errors:
                formatted.append(f"ERROR: {error['message']}")
            formatted.append("")
        
        if warnings:
            formatted.append("WARNINGS:")
            for warning in warnings:
                formatted.append(f"WARNING: {warning['message']}")
    ...
    def get_validation_message(self, issues):
        ...
        if error_count > 0 and warning_count > 0:
            return f"Found {error_count} errors and {warning_count} warnings"
        elif error_count > 0:
            return f"Found {error_count} errors"
        elif warning_count > 0:
            return f"Found {warning_count} warnings"
        else:
            return "No issues found"
...
def validate_export_modifiers(sk_behavior='BASIC'):
    ...
    for collection in bpy.data.collections:
        ...
        # Check for duplicate modifiers (enhanced to handle spaces or no spaces)
...
def check_duplicate_modifiers(original_name, clean_name, item_type):
    ...
    Enhanced to handle modifiers with or without spaces
    """
    issues = []
    
    # Define modifier patterns - now more flexible with spaces
    modifier_patterns = {
        '-dir': r'[\s\-]dir:([^\s]+)',
        '-sep': r'[\s\-]sep',
        '-dk': r'[\s\-]dk',
    ...
    for modifier, pattern in modifier_patterns.items():
        ...
        if modifier == '-dir':
            ...
            if count > 1:
                ...
                issues.append({
                    'type': 'ERROR',
                    'message': f"{item_type} '{clean_name}': {count} duplicate -dir modifiers found: {', '.join(dir_values)}"
                })
            elif count == 1:
    ...
    # Check for conflicting boolean modifiers with flexible spacing
...
def parse_modifiers(name):
    ...
    # Enhanced to handle with or without spaces
    dir_match = re.search(r'[\s\-]dir:([^\s]+)', clean_name, re.IGNORECASE)
    if dir_match:
        modifiers['dir'] = dir_match.group(1).strip()
        # Remove ALL -dir instances from the clean name (with flexible spacing)
        clean_name = re.sub(r'[\s\-]dir:[^\s]+', '', clean_name, flags=re.IGNORECASE).strip()
    
    # Count occurrences of each modifier with flexible spacing
    ...
    # Remove all modifier instances from the clean name (with flexible spacing)
