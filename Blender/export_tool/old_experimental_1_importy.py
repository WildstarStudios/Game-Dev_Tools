##############################################################################
#did not work out willl return later maybe in 2.5 update or 3.0
bl_info = {
...
# ===== ENHANCED DELETE TRACK FILE OPERATOR WITH CONFIRMATION AND TEMPLATE GENERATION =====
class ADVANCED_GLB_OT_delete_track_file(bpy.types.Operator):
    ...
    bl_description = "Delete the export tracking file for this blend file and optionally generate a new template"
    bl_options = {'REGISTER'}
    
    # NEW: Option to generate template after deletion
    ...
    def draw(self, context):
        ...
        # NEW: Template generation option
        layout.prop(self, "generate_template")
        if self.generate_template:
            layout.label(text="A new template will be generated.", icon='FILE_NEW')
    ...
    def execute(self, context):
        ...
        # NEW: Generate template if requested
        if self.generate_template:
...
# ===== NEW DUPLICATE FIX OPERATOR =====
class ADVANCED_GLB_OT_fix_duplicate_modifiers(bpy.types.Operator):
    bl_idname = "advanced_glb.fix_duplicate_modifiers"
    bl_label = "Fix Duplicate Modifiers"
    bl_description = "Automatically fix duplicate modifiers in names"
    bl_options = {'REGISTER', 'UNDO'}
    
    fix_directories: BoolProperty(
        name="Fix Directory Duplicates",
        default=False,
        description="WARNING: This will choose the first -dir modifier. Manual review recommended!"
    )
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(text="Fix duplicate modifiers?", icon='MODIFIER')
        
        # Show what will be fixed
        duplicates = self.find_all_duplicates()
        
        if not duplicates:
            layout.label(text="No duplicate modifiers found!", icon='INFO')
            return
        
        layout.label(text="Items with duplicate modifiers:")
        box = layout.box()
        
        for item_name, dup_info in list(duplicates.items())[:8]:  # Show first 8
            box.label(text=f"• {item_name}")
            for mod, count in dup_info.items():
                if count > 1:
                    box.label(text=f"  - {mod}: {count} duplicates")
        
        if len(duplicates) > 8:
            box.label(text=f"... and {len(duplicates) - 8} more")
        
        # Directory warning
        if any(any(mod.startswith('-dir') for mod in dup_info.keys()) for dup_info in duplicates.values()):
            layout.separator()
            layout.label(text="⚠️ Directory duplicates found!", icon='ERROR')
            layout.prop(self, "fix_directories")
            if self.fix_directories:
                layout.label(text="First -dir modifier will be used", icon='QUESTION')
    
    def execute(self, context):
        fixes_applied = self.fix_all_duplicates()
        
        if fixes_applied:
            self.report({'INFO'}, f"Fixed {fixes_applied} items with duplicate modifiers")
        else:
            self.report({'INFO'}, "No duplicate modifiers needed fixing")
        
        return {'FINISHED'}
    
    def find_all_duplicates(self):
        """Find all duplicate modifiers across collections and objects"""
        duplicates = {}
        
        # Check collections
        for collection in bpy.data.collections:
            dup_info = self.check_item_duplicates(collection.name)
            if dup_info:
                duplicates[collection.name] = dup_info
        
        # Check objects
        for obj in bpy.data.objects:
            dup_info = self.check_item_duplicates(obj.name)
            if dup_info:
                duplicates[obj.name] = dup_info
        
        # Check scene filename
        scene_props = bpy.context.scene.advanced_glb_props
        dup_info = self.check_item_duplicates(scene_props.scene_export_filename)
        if dup_info:
            duplicates[f"Scene: {scene_props.scene_export_filename}"] = dup_info
        
        return duplicates
    
    def check_item_duplicates(self, name):
        """Check a single name for duplicate modifiers"""
        modifier_patterns = {
            '-dir': r'-dir:([^\s]+)',
            '-sep': r'-sep',
            '-dk': r'-dk',
        ...
        for modifier, pattern in modifier_patterns.items():
            ...
            count = len(matches)
            if count > 1:
                duplicates[modifier] = count
        
        return duplicates if duplicates else None
    
    def fix_all_duplicates(self):
        """Fix all duplicate modifiers"""
        fixes_applied = 0
        
        # Fix collections
        for collection in bpy.data.collections:
            if self.fix_item_duplicates(collection):
                fixes_applied += 1
        
        # Fix objects
        for obj in bpy.data.objects:
            if self.fix_item_duplicates(obj):
                fixes_applied += 1
        
        # Fix scene filename
        scene_props = bpy.context.scene.advanced_glb_props
        original_name = scene_props.scene_export_filename
        fixed_name = self.fix_name_duplicates(original_name)
        if fixed_name != original_name:
            scene_props.scene_export_filename = fixed_name
            fixes_applied += 1
        
        return fixes_applied
    
    def fix_item_duplicates(self, item):
        """Fix duplicates for a single item"""
    ...
    def fix_name_duplicates(self, name):
        """Fix duplicate modifiers in a name"""
        if not name:
            return name
        
        # Parse to get clean name and modifiers
        clean_name, modifiers = parse_modifiers(name)
        
        # Rebuild name without duplicates
        new_name = clean_name
        
        # Add modifiers back (without duplicates)
        if modifiers['dir'] and (self.fix_directories or len(re.findall(r'-dir:([^\s]+)', name)) == 1):
            new_name += f" -dir:{modifiers['dir']}"
...
# ===== NEW TRACK FILE TEMPLATE GENERATION =====
def generate_track_file_template():
    ...
    try:
        ...
        settings_key = f"{scene_props.export_path}|{scene_props.export_scope}|{scene_props.export_mode}"
        ...
        if save_track_data(track_data):
            print(f"✅ Generated new tracking template for: {blend_name}")
            return True
        else:
        ...
        print(f"❌ Failed to generate tracking template: {str(e)}")
        return False

# ===== ENHANCED VALIDATE EXPORT MODIFIERS OPERATOR =====
class ADVANCED_GLB_OT_validate_export_modifiers(bpy.types.Operator):
    ...
    bl_description = "Check for incompatible modifier combinations, duplicates, and validate -sk usage"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        self.show_validation_report(context, issues)
    ...
    def show_validation_report(self, context, issues):
        """Show detailed validation report with fix options"""
        def draw_report(self, context):
            ...
            # Show fix duplicates button if there are duplicates
            has_duplicates = any('duplicate' in issue.get('message', '').lower() for issue in issues)
            if has_duplicates:
                row = layout.row()
                row.operator("advanced_glb.fix_duplicate_modifiers", text="Fix Duplicates", icon='MODIFIER')
                layout.separator()
            ...
            if errors:
                ...
                for issue in errors:
                    error_box.label(text=f"• {issue['message']}")
            ...
            if warnings:
                ...
                for issue in warnings:
                    warning_box.label(text=f"• {issue['message']}")
...
def validate_export_modifiers(sk_behavior='BASIC'):
    ...
    for collection in bpy.data.collections:
        ...
        duplicate_issues = check_duplicate_modifiers(collection.name, clean_name, "Collection")
    ...
    for modifier, pattern in modifier_patterns.items():
        ...
        if modifier == '-dir':
            ...
            if count > 1:
                issues.append({
                    'type': 'ERROR',
                    'message': f"{item_type} '{clean_name}': {count} duplicate -dir modifiers found (only first one will be used)"
                })
            elif count == 1:
                ...
                if not is_valid_directory_name(dir_value):
                    issues.append({
                        'type': 'ERROR',
                        'message': f"{item_type} '{clean_name}': -dir value '{dir_value}' contains invalid directory characters"
                    })
        else:
            ...
            if count > 1:
                issues.append({
                    'type': 'WARNING',
                    'message': f"{item_type} '{clean_name}': {count} duplicate {modifier} modifiers found (redundant)"
                })
    
    # Check for conflicting boolean modifiers
    boolean_modifiers = ['-sep', '-dk', '-sk', '-anim']
    modifier_counts = {mod: len(re.findall(modifier_patterns[mod], original_name)) for mod in boolean_modifiers}
    
    # Check for -dk and -sk together (always an error)
    if modifier_counts['-dk'] > 0 and modifier_counts['-sk'] > 0:
        issues.append({
            'type': 'ERROR',
            'message': f"{item_type} '{clean_name}': -dk and -sk cannot be used together (conflicting exclusion)"
        })
    
    # Check for -sep and -sk together (always an error)
    if modifier_counts['-sep'] > 0 and modifier_counts['-sk'] > 0:
        issues.append({
            'type': 'ERROR',
            'message': f"{item_type} '{clean_name}': -sep and -sk cannot be used together (conflicting collection behavior)"
        })
...
class ADVANCED_GLB_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        version_box.label(text="🚀 Version 2.4.0 dev5", icon='EXPERIMENTAL')
        ...
        # NEW: Enhanced validation with duplicate fixing
        validation_row = mod_box.row()
        validation_row.operator("advanced_glb.validate_export_modifiers", icon='CHECKMARK')
        validation_row.operator("advanced_glb.fix_duplicate_modifiers", text="Fix Duplicates", icon='MODIFIER')
...
def parse_modifiers(name):
    ...
    if dir_match:
        ...
        clean_name = re.sub(r'-dir:[^\s]+', '', clean_name).strip()
    
    # Count occurrences of each modifier
    sk_count = len(re.findall(r'-sk', clean_name))
    dk_count = len(re.findall(r'-dk', clean_name))
    sep_count = len(re.findall(r'-sep', clean_name))
    anim_count = len(re.findall(r'-anim', clean_name))
    ...
    clean_name = re.sub(r'-sep', '', clean_name).strip()
    clean_name = re.sub(r'-dk', '', clean_name).strip()
    clean_name = re.sub(r'-sk', '', clean_name).strip()
    clean_name = re.sub(r'-anim', '', clean_name).strip()
...
def register():
    ...
    bpy.utils.register_class(ADVANCED_GLB_OT_fix_duplicate_modifiers)
...
def unregister():
    ...
    bpy.utils.unregister_class(ADVANCED_GLB_OT_fix_duplicate_modifiers)
