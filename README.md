# Blender-And-Godot-tools
a set of blender and godot tools to help me and others with game dev

To Do, Blender:
- ✅ Export 2.4 Update.
- ✅ Export 2.4.1 Update
- Versions 2.4+ include important path fixes for microsoft computers and cloud storage environments. If you experience export directory errors, please upgrade to the latest version.
- ✅ Lod Tool 1.0 Release.
- ❌ Lod Tool 1.0.1 Update
- ✅ Viewport Navigator 1.1 Update.
- ❌ More Tool Ideas.

To Do, Godot:
- ❌ Auto Instancer Plugin (idea)
